...
public class SinglePathState : IState
{
    ...
    {
        owner = stateMachine;
        Debug.Log("Entered Single Path State");
    ...
    public void CalculateAndSpawnNext()
    {
        ...
        Vector2 nextplatformPos = CalculateNextPlatformPosition(owner.lastSpawnedPlatform, spawnGem);

        // Finally, spawn the new platform
        owner.lastSpawnedPlatform = owner.SpawnPlatformAt(nextplatformPos, nextPlatformTag, true);
    }
